...
# self._storage = micrograd_kernel.eigen_alloc_f32(
#     n := len(iterlist := list(iterable))  # self._fmt,
# )
# # self._storage = micrograd_kernel.eigen_alloc(
#     self.dtype.fmt, n := len(iterlist := list(iterable))
# )
# micrograd_kernel.copy_in(self._storage, iterlist, n)
...
class Buffer:
    ...
    @classmethod
    def _filled(cls: type["Buffer"], dtype: DTypeLike, num_elem: int, val: int | float) -> "Buffer":
        out_dtype = to_dtype(dtype)
        valiter = (val for _ in range(num_elem)) if num_elem != 0 else []
        buff: "Buffer" = cls.__new__(cls)
