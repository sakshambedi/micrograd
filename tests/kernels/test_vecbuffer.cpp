...
TEST(VecBufferTest, DoublePrecisionOperations) {
  ...
  binary_kernel<double, AddOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_DOUBLE_EQ(result[i], vec1[i] + vec2[i]);
  }

  binary_kernel<double, SubOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_DOUBLE_EQ(result[i], vec1[i] - vec2[i]);
  }

  binary_kernel<double, MulOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_DOUBLE_EQ(result[i], vec1[i] * vec2[i]);
  }

  binary_kernel<double, DivOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_DOUBLE_EQ(result[i], vec1[i] / vec2[i]);
  }
}
...
TEST(VecBufferTest, IntegerOperations) {
  ...
  binary_kernel<int32_t, AddOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_EQ(result[i], vec1[i] + vec2[i]);
  }

  binary_kernel<int32_t, SubOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_EQ(result[i], vec1[i] - vec2[i]);
  }

  binary_kernel<int32_t, MulOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_EQ(result[i], vec1[i] * vec2[i]);
  }

  binary_kernel<int32_t, DivOp>(vec1.data(), vec2.data(), result.data(), size);
  for (size_t i = 0; i < size; ++i) {
    EXPECT_EQ(result[i], vec1[i] / vec2[i]);
  }
}
...
TEST(VecBufferTest, HalfPrecision) {
  ...
  for (size_t i = 0; i < size; ++i) {
    // Need to convert to float for comparison due to half precision limitations
    float expected = static_cast<float>(vec1[i]) + static_cast<float>(vec2[i]);
