...
TEST(DTypeTest, AlternativeTypeNames) {
  EXPECT_EQ(dtype_from_string("half"), DType::FLOAT16);
  EXPECT_EQ(dtype_from_string("float"), DType::FLOAT32);
  EXPECT_EQ(dtype_from_string("double"), DType::FLOAT64);
}

// Test NumPy-style format codes
TEST(DTypeTest, NumPyFormatCodes) {
  EXPECT_EQ(dtype_from_string("?"), DType::BOOL);
  EXPECT_EQ(dtype_from_string("b"), DType::INT8);
  EXPECT_EQ(dtype_from_string("B"), DType::UINT8);
  EXPECT_EQ(dtype_from_string("h"), DType::INT16);
  EXPECT_EQ(dtype_from_string("H"), DType::UINT16);
  EXPECT_EQ(dtype_from_string("i"), DType::INT32);
  EXPECT_EQ(dtype_from_string("I"), DType::UINT32);
  EXPECT_EQ(dtype_from_string("q"), DType::INT64);
  EXPECT_EQ(dtype_from_string("Q"), DType::UINT64);
  EXPECT_EQ(dtype_from_string("e"), DType::FLOAT16);
  EXPECT_EQ(dtype_from_string("f"), DType::FLOAT32);
  EXPECT_EQ(dtype_from_string("d"), DType::FLOAT64);
}
